...
export const IMPORT_TABLES: Record<TableKey, TableDef> = {
  ...
  kundenretouren: {
    label: "Kundenretouren",
    example: { orderid: "ORDER-1001", datum: "14.07.2026", menge: "1", erstattung: "59,99", zustand: "USED" },
    fields: [
      field("orderid", "OrderID des Verkaufs", ["orderid", "order-id", "verkauf"], true, { description: "Vorhandene Verkaufs- oder Legacy-OrderID.", format: "Text", example: "ORDER-1001" }),
      field("datum", "Meldedatum", ["meldedatum", "datum", "date"], true, { description: "Datum der Kundenmeldung.", format: "TT.MM.JJJJ oder JJJJ-MM-TT", example: "14.07.2026" }),
      field("eingangsdatum", "Eingangsdatum", ["eingangsdatum", "received at"], false, { description: "Optionaler physischer Eingang; die Bestandsbuchung bleibt ein bestätigter Workflow-Schritt.", format: "Datum", example: "18.07.2026" }),
      field("menge", "Menge", ["menge", "quantity"], false, { description: "Historische Menge; relationale Allocationen werden nur bei eindeutiger Zuordnung erzeugt.", format: "Positive Ganzzahl", example: "1" }),
      field("problem", "Rückgabegrund", ["problem", "problemart", "grund"], false, { description: "Operativer Rückgabegrund.", format: "Text", example: "Widerruf" }),
      field("zustand", "Artikelzustand", ["zustand", "condition"], false, { description: "Kanonischer Artikelzustand.", format: "NEW, OPEN_BOX, REFURBISHED, USED oder DEFECTIVE", example: "USED" }),
      field("erstattung", "Erstattung", ["erstattung", "refund"], false, { description: "Erstattungsbetrag brutto.", format: "EUR-Dezimalzahl", example: "59,99" }),
      field("zusatzkosten", "Zusatzkosten", ["zusatzkosten", "additional costs"], false, { description: "Zusätzliche Kosten ohne Rückversand.", format: "EUR-Dezimalzahl", example: "2,00" }),
      field("ruecksendekosten", "Rücksendekosten", ["rücksendekosten", "ruecksendekosten", "porto"], false, { description: "Kosten der Kundenrücksendung.", format: "EUR-Dezimalzahl", example: "6,99" }),
      field("tracking", "Trackingnummer", ["tracking", "trackingnummer"], false, { description: "Optionale Sendungskennung.", format: "Text", example: "003404341234" }),
      field("kommentar", "Notizen", ["kommentar", "notiz", "notes"], false, { description: "Optionale operative Notiz.", format: "Text", example: "Verpackung beschädigt" }),
    ],
  },
  lieferantenretouren: {
    label: "Lieferantenretouren",
    example: { einkaufsnummer: "E-26-0042", lagerid: "L-26-0091", menge: "1", grund: "Falschlieferung", frist: "31.07.2026", erwartete_erstattung: "34,99" },
    fields: [
      field("einkaufsnummer", "Einkaufsnummer", ["einkaufsnummer", "purchase number"], true, { description: "Vorhandene StorageX-Einkaufsnummer.", format: "E-YY-NNNN", example: "E-26-0042" }),
      field("lagerid", "LagerID", ["lagerid", "lager-id", "inventory number"], true, { description: "Konkretes eigenes Lot der Einkaufsposition.", format: "L-YY-NNNN", example: "L-26-0091" }),
      field("menge", "Menge", ["menge", "quantity"], true, { description: "Geplante Rückgabemenge.", format: "Positive Ganzzahl", example: "1" }),
      field("bucket", "Bestands-Bucket", ["bucket", "bestand"], false, { description: "Quelle der später bestätigten Versandbewegung.", format: "AVAILABLE, RESERVED, INSPECTION oder DEFECTIVE", example: "AVAILABLE" }),
      field("grund", "Rückgabegrund", ["grund", "reason"], true, { description: "Grund der Lieferantenretoure.", format: "Text", example: "Falschlieferung" }),
      field("meldedatum", "Meldedatum", ["meldedatum", "datum", "requested at"], false, { description: "Planungs-/Anfragedatum.", format: "Datum", example: "15.07.2026" }),
      field("frist", "Rückgabefrist", ["frist", "rückgabefrist", "rueckgabefrist"], false, { description: "Bestätigte Rückgabefrist.", format: "Datum", example: "31.07.2026" }),
      field("rma", "RMA-/Referenznummer", ["rma", "rma nummer", "referenz"], false, { description: "Lieferantenreferenz.", format: "Text", example: "RMA-1042" }),
      field("erwartete_erstattung", "Erwartete Erstattung", ["erwartete erstattung", "expected refund"], false, { description: "Erwarteter Gutschriftsbetrag.", format: "EUR-Dezimalzahl", example: "34,99" }),
      field("versandkosten", "Versandkosten", ["versandkosten", "shipping cost"], false, { description: "Geplante oder bekannte Rücksendekosten.", format: "EUR-Dezimalzahl", example: "6,99" }),
      field("notiz", "Notizen", ["notiz", "kommentar", "notes"], false, { description: "Optionale operative Notiz.", format: "Text", example: "Originalverpackt" }),
    ],
  },
  konsignation: {
    label: "Konsignation",
  ...
  aufgaben: {
    label: "Aufgaben",
    example: { aufgabe: "Wareneingang prüfen", anmerkung: "Lieferung auf Beschädigungen prüfen", bereich: "Lager", prioritaet: "Mittel", status: "Offen", frist: "18.07.2026", bearbeiter_email: "team@example.com", teamaufgabe: "Ja" },
    fields: [
      field("aufgabe", "Titel", ["aufgabe", "titel", "task"], true, { description: "Kurzer, handlungsorientierter Aufgabentitel.", format: "Text, max. 300 Zeichen", example: "Wareneingang prüfen" }),
      field("anmerkung", "Beschreibung", ["anmerkung", "beschreibung", "kommentar", "notiz"], false, { description: "Optionaler Kontext und Akzeptanzkriterien.", format: "Text, max. 5000 Zeichen", example: "Lieferung auf Beschädigungen prüfen" }),
      field("bereich", "Bereich", ["bereich", "area", "kategorie"], false, { description: "Konfigurierbarer operativer Bereich.", format: "Text", example: "Lager" }),
      field("prioritaet", "Priorität", ["priorität", "prioritaet", "prio", "priority"], false, { description: "Arbeitspriorität.", format: "Niedrig, Mittel, Hoch oder Dringend", example: "Mittel" }),
      field("status", "Status", ["status"], false, { description: "Aktueller Workflowstatus.", format: "Offen, In Arbeit, Erledigt oder Abgebrochen", example: "Offen" }),
      field("frist", "Frist", ["frist", "deadline", "fällig", "faellig"], false, { description: "Optionale Fälligkeit.", format: "TT.MM.JJJJ oder JJJJ-MM-TT", example: "18.07.2026" }),
      field("bearbeiter_email", "Bearbeiter-E-Mail", ["bearbeiter-e-mail", "bearbeiter email", "assignee email", "zustaendig", "zuständig"], false, { description: "E-Mail eines aktiven Organisationsmitglieds; unbekannte Nutzer blockieren den Import.", format: "E-Mail-Adresse", example: "team@example.com" }),
      field("teamaufgabe", "Teamaufgabe", ["teamaufgabe", "team task", "team"], false, { description: "Kennzeichnet eine für das Team sichtbare Aufgabe.", format: "Ja/Nein", example: "Ja" }),
    ],
  },
  ausgaben: {
  ...
  gebuehrenregeln: {
    ...
    example: {
      ...
      prioritaet: "10",
      quelle: "Manuell geprüft",
      aktiv: "Ja",
    },
    fields: [
      field("plattform", "Plattform", ["plattform", "platform"], true, { description: "Name einer vorhandenen Plattform.", format: "Text; eindeutige Zuordnung", example: "Kaufland" }),
      field("marktplatzkonto", "Marktplatzkonto", ["marktplatzkonto", "marketplace account"], false, { description: "Optionaler Anzeigename eines Accounts derselben Plattform.", format: "Text; eindeutige Zuordnung", example: "Kaufland Deutschland" }),
      field("kategorie", "Kategorie", ["kategorie", "category"], false, { description: "Optionale fachliche Gebührenkategorie.", format: "Text", example: "Elektronik" }),
      field("zustand", "Artikelzustand", ["zustand", "condition"], false, { description: "Optionaler kanonischer Artikelzustand.", format: "NEW, OPEN_BOX, REFURBISHED, USED oder DEFECTIVE", example: "NEW" }),
      field("gueltig_ab", "Gültig ab", ["gültig ab", "gueltig ab", "valid from"], true, { description: "Erster Gültigkeitstag der Regel.", format: "TT.MM.JJJJ oder JJJJ-MM-TT", example: "01.07.2026" }),
      field("gueltig_bis", "Gültig bis", ["gültig bis", "gueltig bis", "valid until"], false, { description: "Optionaler letzter Gültigkeitstag.", format: "TT.MM.JJJJ oder JJJJ-MM-TT", example: "" }),
      field("prozent", "Prozentuale Gebühr", ["prozent", "percentage", "gebühr prozent", "gebuehr prozent"], true, { description: "Prozentuale Komponente.", format: "Dezimalzahl in Prozent", example: "11,00" }),
      field("fix", "Fixe Gebühr", ["fix", "fixe gebühr", "fixed fee"], false, { description: "Fixe Komponente je Regel.", format: "EUR-Dezimalzahl", example: "0,35" }),
      field("minimum", "Mindestwert", ["minimum", "mindestwert", "minimum fee"], false, { description: "Optionaler Mindestgebührenwert.", format: "EUR-Dezimalzahl", example: "" }),
      field("maximum", "Maximalwert", ["maximum", "maximalwert", "maximum fee"], false, { description: "Optionaler Maximalgebührenwert.", format: "EUR-Dezimalzahl", example: "" }),
      field("werbung", "Werbegebühr (%)", ["werbung", "werbegebühr", "advertising"], false, { description: "Optionale Werbegebühr in Prozent.", format: "Dezimalzahl in Prozent", example: "0" }),
      field("zahlungsgebuehr", "Zahlungsgebühr (%)", ["zahlungsgebühr", "zahlungsgebuehr", "payment fee"], false, { description: "Optionale Zahlungsgebühr in Prozent.", format: "Dezimalzahl in Prozent", example: "0" }),
      field("ust_behandlung", "USt-Behandlung", ["ust-behandlung", "ust behandlung", "vat treatment"], false, { description: "Umsatzsteuerbehandlung der Gebühr.", format: "INCLUDED, EXCLUDED oder UNKNOWN", example: "EXCLUDED" }),
      field("prioritaet", "Priorität", ["priorität", "prioritaet", "priority"], false, { description: "Höhere Werte gewinnen bei gleicher Spezifität.", format: "Ganzzahl", example: "10" }),
      field("quelle", "Quelle", ["quelle", "source"], false, { description: "Nachvollziehbare manuelle Quelle oder Referenz.", format: "Text", example: "Manuell geprüft" }),
      field("aktiv", "Aktiv", ["aktiv", "active"], false, { description: "Ob die Regel nach Import aktiv ist.", format: "Ja/Nein", example: "Ja" }),
    ],
  },
};
