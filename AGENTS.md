...
- Expose narrowly scoped, typed server read contracts for module lists, reference data, details, and history. The client-provided organization ID is a cache namespace, never authorization. By default API routes resolve the active organization through `resolveApiOrgContext`.
- Approved performance exception for normal read contracts: a server-verified, signed session snapshot of active organization and role may be used for at most 30 minutes, provided its refresh, organization switch, sign-out, expiry, and tenant-isolation behavior are covered by tests. Never accept browser storage, a client parameter, or process-local memory as authorization. On an expired snapshot, refresh against the database or retain the fresh check; do not simulate freshness. Membership or role revocation may therefore take up to 30 minutes to affect ordinary reads, as explicitly accepted for this phase.
- Every write and all inventory, financial, security-sensitive, export, entitlement, and administrative operations must continue to use a fresh server-side membership/role check through the established `requireOrg`/`resolveApiOrgContext` path. RLS remains mandatory for all tenant queries.
...
- Migrate one tab at a time. The shared foundation is complete. The approved next pilot is `/verkauf`, with no visible UI/UX changes in its first performance round; `/lager` is deferred. Do not migrate several business tabs in one unreviewable change.
- Read `docs/plans/2026-08-01-001-hybrid-spa-performance-plan.md` and `docs/plans/2026-08-09-002-sales-read-performance-pilot-plan.md` before implementing this architecture. Use the current prompts under `docs/codex-prompts/` in their stated sequence.
